#!/usr/bin/env node
/**
 * Phoenix Browser Relay — MCP Server (v2 自启动版)
 *
 * 核心改进：
 ...
 *       "type": "stdio",
 *       "command": "node",
 *       "args": ["/ABSOLUTE/PATH/TO/phoenix-mcp/index.js"]
 *     }
 *   }
 *
 * 环境变量（可选覆盖）：
...
const EXTENSION_DIR = path.join(PROJECT_DIR, "phoenix-browser-relay");
...
// ============================================================
function log(msg) {
  process.stderr.write(`[phoenix-mcp] ${msg}\n`);
}
...
async function listAndAttach() {
  ...
  if (targets.length === 0) {
    throw new Error(
      "没有发现任何浏览器标签页。请确保：\n" +
      "1. Chrome 浏览器已打开\n" +
      "2. Phoenix Browser Relay Extension 已安装并显示绿色 Connected 状态\n" +
...
const TOOLS = [
  {
    name: "browser_status",
    description: "检查 Phoenix 浏览器连接状态：Extension 是否已连接、加密是否激活、有哪些标签页。首次调用会自动启动 Relay Server。",
...
const server = new Server(
  { name: "phoenix-browser-relay", version: "2.0.0" },
  { capabilities: { tools: {} } }
);
...
main().catch((err) => {
  process.stderr.write(`Phoenix MCP Server 启动失败: ${err.message}\n`);
  process.exit(1);
});
