...
const PORT_NAME = 'laoliu-keepalive'
  ...
  try {
    await chrome.scripting.executeScript({
      ...
      func: (portName, interval) => {
        if (window._laoliuKeepAlive) return
        ...
        function connect() {
          try {
            ...
            window._laoliuKeepAlive = { port, timer }
...
export async function stopKeepAlive(tabId) {
  ...
  try {
    await chrome.scripting.executeScript({
      target: { tabId },
      func: () => {
        if (window._laoliuKeepAlive) {
          clearInterval(window._laoliuKeepAlive.timer)
          try { window._laoliuKeepAlive.port.disconnect() } catch { /* noop */ }
          window._laoliuKeepAlive = null
        }
      },
    })
