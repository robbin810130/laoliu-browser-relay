...
export async function extActionMask(tabId, params) {
  ...
  const results = await chrome.scripting.executeScript({
    target: { tabId },
    func: (maskState) => {
      const MASK_ID = 'laoliu-action-mask'
      ...
      if (!host) {
        ...
        style.textContent = `
          ...
          .glow-border {
            ...
            animation: laoliu-glow 1.5s ease-in-out infinite alternate;
          }
          @keyframes laoliu-glow {
            from { opacity: 0.7; }
            to { opacity: 1; }
          }
          .status-bar {
          ...
          .status-dot {
            ...
            background: #6366f1;
            animation: laoliu-pulse 1s ease-in-out infinite alternate;
          }
          @keyframes laoliu-pulse {
            from { opacity: 0.5; }
            to { opacity: 1; }
          }
        `
        ...
        bar.innerHTML = '<span class="status-dot"></span><span>老六正在操作…</span>'
