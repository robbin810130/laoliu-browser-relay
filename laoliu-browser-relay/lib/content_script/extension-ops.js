  ...
  const results = await chrome.scripting.executeScript({
    ...
    func: (options) => {
      ...
      for (const el of candidates) {
        ...
        el.setAttribute('data-laoliu-idx', String(idx))
  ...
  const results = await chrome.scripting.executeScript({
    ...
    func: (p) => {
      ...
      if (p.index != null) {
        ...
        el = deepQuery(document, `[data-laoliu-idx="${idx}"]`)
      } else if (p.selector) {
...
async function showClickRipple(tabId, x, y) {
  await chrome.scripting.executeScript({
    target: { tabId },
    func: (cx, cy) => {
      if (!document.getElementById('laoliu-ripple-style')) {
        const s = document.createElement('style')
        s.id = 'laoliu-ripple-style'
        s.textContent = '@keyframes laoliu-r{0%{transform:scale(0);opacity:1}100%{transform:scale(2.5);opacity:0}}'
      ...
      Object.assign(d.style, {
        ...
        zIndex: '2147483647', animation: 'laoliu-r .8s ease-out forwards',
      })
