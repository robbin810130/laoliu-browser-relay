...
function notifyError(title, message) {
  chrome.notifications.create('laoliu-relay-error', {
    type: 'basic',
    iconUrl: 'icons/icon128.png',
    title: `老六 Relay: ${title}`,
    message,
    priority: 2,
  }, () => {
    if (chrome.runtime.lastError) {
      console.warn('[laoliu-relay] notification create failed:', chrome.runtime.lastError.message)
    }
  })
}
...
export function trySendToRelay(payload) {
  try {
    ...
    console.debug('[laoliu-relay] trySendToRelay failed:', err)
  }
}
...
export async function toggle() {
  ...
  try {
    ...
    console.warn('[laoliu-relay] initial connect failed:', message)
