...
function notifyError(title, message) {
  chrome.notifications.create('phoenix-relay-error', {
    type: 'basic',
    iconUrl: 'icons/icon128.png',
    title: `Phoenix Relay: ${title}`,
    message,
    priority: 2,
  }, () => {
    if (chrome.runtime.lastError) {
      console.warn('[phoenix-relay] notification create failed:', chrome.runtime.lastError.message)
    }
  })
}
...
export function trySendToRelay(payload) {
  try {
    ...
    console.debug('[phoenix-relay] trySendToRelay failed:', err)
  }
}
...
export async function toggle() {
  ...
  try {
    ...
    console.warn('[phoenix-relay] initial connect failed:', message)
