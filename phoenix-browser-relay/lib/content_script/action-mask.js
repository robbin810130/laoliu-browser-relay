...
export async function extActionMask(tabId, params) {
  ...
  const results = await chrome.scripting.executeScript({
    target: { tabId },
    func: (maskState) => {
      const MASK_ID = 'phoenix-action-mask'
      ...
      if (!host) {
        ...
        style.textContent = `
          ...
          .glow-border {
            ...
            animation: phoenix-glow 1.5s ease-in-out infinite alternate;
          }
          ...
          .status-dot {
            ...
            background: #6366f1;
            animation: phoenix-pulse 1s ease-in-out infinite alternate;
          }
        ...
        bar.innerHTML = '<span class="status-dot"></span><span>Phoenix is working…</span>'
