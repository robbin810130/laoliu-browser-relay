...
const PORT_NAME = 'phoenix-keepalive'
  ...
  try {
    await chrome.scripting.executeScript({
      ...
      func: (portName, interval) => {
        if (window._phoenixKeepAlive) return
        ...
        function connect() {
          try {
            ...
            window._phoenixKeepAlive = { port, timer }
...
export async function stopKeepAlive(tabId) {
  ...
  try {
    await chrome.scripting.executeScript({
      target: { tabId },
      func: () => {
        if (window._phoenixKeepAlive) {
          clearInterval(window._phoenixKeepAlive.timer)
          try { window._phoenixKeepAlive.port.disconnect() } catch { /* noop */ }
          window._phoenixKeepAlive = null
        }
      },
    })
